...
function PacMan() // kalsa pacman 
{
    ...
    this.update = function () // updejtuje pozycje pacmana
    {
        ...
        for (let i = 0; i < wall.length; i++) {
            if (bol = p.colision(wall[i]) == 1)
                break;
        }
        if (bol == 1 && (p.xspeed > oldx || p.xspeed < oldx) && (p.yspeed > oldy || p.yspeed < oldy) && (oldposx == p.x && oldposy == p.y)) {
            p.dir(oldx, oldy);
            bol = 0;
        }
    ...
    this.show = function () // rysowanie pacmana
    {
        ...
        if (stan >= 3 && stan < 5) {
            if (currentDirection == 1)
                image(pacman_GoraP, this.x, this.y, scl, scl);
            if (currentDirection == 2)
                image(pacman_PrawoP, this.x, this.y, scl, scl);
            if (currentDirection == 3)
                image(pacman_DolP, this.x, this.y, scl, scl);
            if (currentDirection == 4)
                image(pacman_LewoP, this.x, this.y, scl, scl);
            stan += 1;
            return;
        }
        if (stan >= 5 && stan < 9) {
    ...
    this.dir = function (x, y) // zmienia kierunek poruszania sie pacmana -  w zaleznosci od wcisnietej strzalki
    {
        oldx = this.xspeed;
        oldy = this.yspeed;
        this.xspeed = x;
    ...
    this.colision = function (pos) {
        if ((((this.x + (scl - scl * factor_speed)) >= (pos.x)) && ((this.x - (scl - scl * factor_speed)) <= (pos.x + pos.width - scl))) && (((this.y + (scl - scl * factor_speed)) >= (pos.y)) && ((this.y - (scl - scl * factor_speed)) <= (pos.y + pos.height - scl))))
...
function Ghost(start_x, start_y, xs, type) {
    ...
    this.update = function () // updejtuje pozycje pacmana
    {
        ...
        for (let i = 0; i < wall.length; i++) {
            if (bol = this.colision(wall[i]) == 1)
                break;
        }
        if (bol == 1 && (this.xspeed > this.oldx_ghost || this.xspeed < this.oldx_ghost) && (this.yspeed > this.oldposy_ghost || this.yspeed < this.oldposy_ghost) && (this.oldposx_ghost == this.x && this.oldposy_ghost == this.y)) {
    ...
    this.show = function (photo) // rysowanie pacmana
    {
        if ((ifScared) && (this.ownIfScared))
            photo = ghost_alt_img;
    ...
    this.colision = function (pos) {
        if ((((this.x + (scl - scl * this.ownFactorSpeed)) >= (pos.x)) && ((this.x - (scl - scl * this.ownFactorSpeed)) <= (pos.x + pos.width - scl))) && (((this.y + (scl - scl * this.ownFactorSpeed)) >= (pos.y)) && ((this.y - (scl - scl * this.ownFactorSpeed)) <= (pos.y + pos.height - scl))))
        //if ((((this.x) >= (pos.x)) && ((this.x) <= (pos.x + pos.width - scl))) && (((this.y) >= (pos.y)) && ((this.y) <= (pos.y + pos.height - scl))))
        //if((this.y == pos.y && this.x == pos.x))
        {
            if (this.xspeed == 1) {
